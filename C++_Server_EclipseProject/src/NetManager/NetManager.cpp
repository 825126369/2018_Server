...
NetOutStream:: NetOutStream(const int command,const int Length,const unsigned char* data)
{
		...
		int sumLength=msg_head_Length+head_Length+Length+msg_tail_Length;
		unsigned char msgStream[sumLength]={};
		...
		unsigned char msgStream1[msg_Length];
